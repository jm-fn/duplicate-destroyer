...
pub(crate) fn blake2(path: &OsString) -> Result<String> {
...
pub(crate) fn blake2_partial<const LEN: usize>(path: &OsString) -> Result<String> {
    let mut hasher = Blake2b512::new();
...
mod tests {
    ...
    fn get_partial_checksum_test() -> Result<()> {
        ...
        let checksum = blake2_partial::<100>(&OsString::from(file_path));
